...
class AuthenticationController extends Controller
{
    ...
    public function semakIdAwam(Request $request)
    {
        ...
        if ($response_status == 00) {
            ...
            if ($response_msg == "Rekod Pemilik Tidak Dijumpai|Owner Record Not Found") {
                $response_status = "55";
            } else {
                ...
                $headers = array(
                    ...
                    "Cache-Control: no-cache",
                    "Pragma: no-cache",
                    "SOAPAction: http://www.jpj.gov.my/ebid_ws/",
                    "Content-length: " . strlen($xml_post_string),
                ); //SOAPAction: your op URL
                ...
                $doc2->loadXML($response);
                 ...
                 $statusebid = $doc2->getElementsByTagName('respSta')->item(0)->nodeValue;
                if($statusebid!='01'){
                        $userID2 = $doc2->getElementsByTagName('ownerId')->item(0)->nodeValue;
                        $userName2 = $doc2->getElementsByTagName('name')->item(0)->nodeValue;
                        $userCat2 = $doc2->getElementsByTagName('category')->item(0)->nodeValue;
                    }else{
                         $userID2 = $nokp;
                        $userName2 = 'Nama Anda';
                        $userCat2 = '1-ORANG AWAM MALAYSIA (MAL)';
        ...
        } elseif ($response_status == "01") {
            $obj->nokp = $userID2;
            $obj->name = $userName2;
            $obj->category = $userCat2;
        }
    ...
    public function registerIdAwam(Request $req)
    {
        ...
        $obj->status_code = $response_status;
        $obj->status_msg = $response_msg;
    ...
    public function firstTimeLogin(Request $request)
    {
        ...
        // try {
            ...
            $soapUrl = "https://mobile.jpj.gov.my/jpj-revamp-svc-pvr-ws/idm_public_login_authentication"; // asmx URL of WSDL
            ...
            if ($obj->statusCode == 00) {
                ...
                $obj->svr = "228";
               $nokp = $doc->getElementsByTagName('idmpuUsrId')->item(0)->nodeValue;
                if($nokp == '770830055395' or $nokp == '670813086247' or $nokp == '910523085131' or $nokp == '680925135604' or $nokp == '770428055709' or $nokp == '630102135341' or $nokp == '710425105585' or $nokp == '821128016115' or $nokp == '821023135361'  or $nokp == '790904135151' or $nokp == '710722035675' or $nokp == '620811115299')
                {
                    $obj->idmpuStaffInd = 'true';
                }else{
                    $obj->idmpuStaffInd = $doc->getElementsByTagName('idmpuStaffInd')->item(0)->nodeValue;
                }


                $existing = User::where('username', $obj->idmpuUsrId)->first(['ref_no', 'password', 'nama', 'nokp', 'username', 'email', 'id']);

                if ($existing) {
                    $existing->ref_no = $obj->idmpuRefNo;
                    $existing->password = Hash::make($obj->idmpuPassword);
                    $existing->nama = $obj->idmpuUserName;
                    $existing->nokp = $obj->idmpuUsrId;
                    $existing->username = $obj->idmpuUsrId;
                    $existing->email = $obj->idmpuUserEmail;
                    $existing->save();
                    Auth::guard()->loginUsingId($existing->id);
                    $token = Auth::user()->createToken("swagger", "web");
                } else {
                    $user = User::create([
                        'username' => $username,
                        'password' => Hash::make($obj->idmpuPassword),
                        'nama' => $obj->idmpuUserName,
                        'nokp' => $username,
                        'email' => $obj->idmpuUserEmail,
                        'ref_no' => $obj->idmpuRefNo,
                    ]);

                    Auth::guard()->loginUsingId($user->id);
                    $token = Auth::user()->createToken("swagger", "web");
                }
                unset($obj->idmpuPassword);
                $obj->bearer_token = $token->plainTextToken;
    ...
    public function soalanKeselamatan(Request $req)
    {
        ...
        $soapUrl = "https://mobile.jpj.gov.my/jpj-revamp-svc-pvr-ws/idm_public_security_question";
        $soapUser = "username";  //  username
        $soapPassword = "password"; // password
        $xml_post_string = '<?xml version="1.0" encoding="utf-8"?>
                                    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:idm="http://www.example.org/idm_public_security_question/">
    ...
    public function changePasswordIdAwam(Request $req)
    {
        ...
        // $jenis = 1;
        // $jenis_data = "JSON";
        $soapUrl = "https://mobile.jpj.gov.my/jpj-revamp-svc-pvr-ws/idm_public_mobile_reset_pwd";
        // LogController::insertDatalog($jenis, $jenis_data, $ip, 'services', $function, $soapUrl, $postdata);
    ...
    public function resetPasswordIdAwam2(Request $req)
    {
      $obj = new stdClass;
      $obj->status = "99";
      $obj->msg = "Fungsi Ini Sedang Dalam Penyelenggaraan. Sebarang Kesulitan Amat Dikesali.|Fungsi Ini Sedang Dalam Penyelenggaraan. Sebarang Kesulitan Amat Dikesali.";
    ...
    public function resetPasswordIdAwam(Request $req)
    {
        ...
        return response()->json($obj);
          ...
          $statusMsg = "Error";
          $status = array( "status_message" => $statusMsg, "connStatus" => 400, "response" => "99", "error" => print_r($th));

          return response()->json($status);
      }
    }

    public function get_maklumat_kenderaan($nokp)
    {
        $maklumat_kenderaan = MaklumatKenderaan::where('nokp', $nokp)->get();
        if ($maklumat_kenderaan->isEmpty()) {
            return response()->json([
                'status_code' => '00',
                'maklumat_kenderaan' => 'Record not found.'
            ]);
        } else {
            return response()->json([
                'status_code' => '01',
                'maklumat_kenderaan' => $maklumat_kenderaan
            ]);
        }
    }

    public function add_maklumat_kenderaan(Request $request)
    {
        $mk = new MaklumatKenderaan();
        $mk->nokp = $request->nokp;
        $mk->no_kenderaan = strtoupper($request->nokenderaan);
        ...
        try {
            ...
            $obj = new stdClass;
            $obj->status_code = '200';
            $obj->status_msg = 'saved';
            $obj->maklumat_kenderaan = $mk;
        } catch (\Throwable $th) {
            $obj = new stdClass;
            $obj->status_code = '422';
            $obj->status_msg = 'unsaved';
        }

        return response()->json($obj);
    }

    public function remove_maklumat_kenderaan($id)
    {
        $mk = MaklumatKenderaan::find($id);
        try {
            $mk->delete();
            ...
            return response()->json([
                'status_code' => 01,
                'status_message' => 'Delete unsuccessfully'
            ]);
        }
    }

    public function get_maklumat_kenderaan2()
    {
        $maklumat_kenderaan = MaklumatKenderaan::all();
        return response()->json($maklumat_kenderaan);
    }
